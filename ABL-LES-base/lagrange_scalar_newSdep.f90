subroutine lagrange_scalar_newSdep(Cs2_Pr,S11,S12,S13,S22,S23,S33)
...
real(kind=rprec)::Tstddev ! Standard deviation of temperature 
...
!-----------------------------------------------------------
! Standard deviation of temperature calculations
!-----------------------------------------------------------
  Tstddev = (sum(theta(1:nx,1:ny,jz)**2)/float(nx*ny)) - &
            (sum(theta(1:nx,1:ny,jz))/float(nx*ny))**2

  Tstddev = sqrt(Tstddev)
!-----------------------------------------------------------
  ...
  ! 1st 10000 iterations, time scale used to integrate is 1 corresponding
  ! to momentum & then the scale is 1 corresponding to temperature
  ! This is so because initially temperature variance is very small and 
  ! can be negative due to numerical errors
  if (jt .lt. 10000) then
  Tn(:,:) = TnLMMM(:,:,jz)
!    Tn = TnLMMM
    ...
    Tn = opftdelta*(Tn**powcoeff)*Tstddev
  end if
...
!   Tn = TnQNNN
  ...
  do jy = 1,ny
    do jx = 1,ld
      Betaclip=max(real(Beta(jx,jy,jz)),real(1.0_rprec/(tf1*tf2)))
...
end subroutine lagrange_scalar_newSdep
