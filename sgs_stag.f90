...
use sgsmodule,only:u_lag,v_lag,w_lag,Cs_opt2,Nu_t,dissip,Pr_t, &
                   u_lag_s,v_lag_s,w_lag_s,tau,SS,SS11,SS22,SS33,SS12,SS13,SS23,CS_O,Sl
!------------------------- Vij Comment begins---------------
   ...
   if((model == 1))then  !For traditional Smagorinsky   ref02
    ...
    elseif ( ((jt.GE.DYN_init).OR.(initu)) .AND. &
      ...
      elseif (model==7) then ! new_scaledep_dynamic
        ...
        ! call lagrange_scalar_newSdep(Cs2_Pr,S11,S12,S13,S22,S23,S33)
        ! This module (lagrange_scalar_newSdep) has different time integration scale 
...
do jy=1,ny
do jx=1,nx
   !! -- NOT Now :MM Attecntion: I Changed the following coeffs
   S(jx,jy) = sqrt(2._rprec*(S11(jx,jy,jz)**2 + S22(jx,jy,jz)**2 +&
        S33(jx,jy,jz)**2 + 2._rprec*(S12(jx,jy,jz)**2 +&
        S13(jx,jy,jz)**2 + S23(jx,jy,jz)**2)))
   
   SS(jx,jy,jz)=S(jx,jy)     
   SS11(jx,jy,jz)=S11(jx,jy,jz)
   SS22(jx,jy,jz)=S22(jx,jy,jz)
   SS33(jx,jy,jz)=S33(jx,jy,jz)
   SS12(jx,jy,jz)=S12(jx,jy,jz)
   SS13(jx,jy,jz)=S13(jx,jy,jz)
   SS23(jx,jy,jz)=S23(jx,jy,jz)
   CS_O(jx,jy,jz)=Cs_opt2(jx,jy,jz)
   Sl(jz)=l(jz)
 
   Nu_t(jx,jy,jz)=S(jx,jy)*Cs_opt2(jx,jy,jz)*(l(jz)**2)
   dissip(jx,jy,jz)=-(S(jx,jy)**3)*Cs_opt2(jx,jy,jz)*(l(jz)**2)
   tau(jx,jy,jz)=Nu_t(jx,jy,jz)*S(jx,jy)     
end do
end do
...
do jx=1,nx
   ...
   txz(jx,jy,jz)=-2._rprec*(1+0.5_rprec*exp(-10._rprec*zz(jz)/1250._rprec))*(const + nu) * S13(jx,jy,jz)
   tyz(jx,jy,jz)=-2._rprec*(1+0.5_rprec*exp(-10._rprec*zz(jz)/1250._rprec))*(const + nu) * S23(jx,jy,jz)
end do
end do
end do
!$comp end parallel do
